...
export function PortfolioSection() {
  ...
  const cardWidth = isMobile ? '92vw' : 'min(1300px, 90vw)'
  const cardHeight = isMobile ? '500px' : '700px'
  ...
  return (
    ...
    >
      ...
      <motion.h2
        ...
        className="relative z-10 mb-20 text-5xl font-light text-[#0E1D3D] md:text-6xl"
      >
        Case Studies
      </motion.h2>

      {/* Carousel Container */}
      <div className="relative h-[550px] w-full md:h-[750px]">
              ...
              <Link href={study.link} className="block h-full">
                <div className="group relative h-full overflow-visible rounded-[20px] shadow-[0_30px_80px_rgba(0,0,0,0.2)]">
                  ...
                  <div className="relative z-[2] h-full">
                    {/* Top Left Content */}
                    <div className="absolute left-10 top-10 max-w-[520px] md:left-[60px] md:top-[60px]">
                      {/* Case Label */}
                      <div className="mb-5 text-[10px] font-medium uppercase tracking-[0.2em] text-white/50 md:text-[11px]">
                        CASE {study.id.toString().padStart(2, '0')} — {labelText}
                      </div>

                      {/* Title */}
                      <h3 className="mb-4 text-[34px] font-light leading-[1.2] text-white md:text-[42px]">
                        {study.title}
                      </h3>

                      {/* Description */}
                      <p className="mb-10 max-w-[480px] text-[15px] leading-[1.6] text-white/85 md:text-[17px]">
                        {study.subtitle}
                      </p>

                      {/* VIEW Button */}
                      <button className="inline-flex items-center rounded-[30px] border-[1.5px] border-white/30 bg-white/[0.08] px-6 py-2.5 text-[10px] font-medium uppercase tracking-[0.15em] text-white backdrop-blur-[10px] transition-all duration-300 hover:-translate-y-0.5 hover:border-white/50 hover:bg-white/[0.15] md:px-8 md:py-3 md:text-[12px]">
                        VIEW
                      </button>
                    </div>
                  </div>
              ...
              <AnimatePresence mode="wait">
                {isActive && (
                  <motion.div
                    ...
                    className="absolute -bottom-12 left-0 right-0 z-[70] mx-auto w-[calc(100%-60px)] max-w-[800px] rounded-2xl bg-white/[0.97] p-7 text-center shadow-2xl backdrop-blur-xl md:-bottom-16 md:w-[calc(100%-120px)] md:p-12"
                  >
                    {/* Quote */}
                    <p className="mb-6 text-[15px] italic leading-relaxed text-gray-600 md:text-[17px]">
                      "{study.quote}"
                    </p>

                    {/* Author */}
                    <div className="mb-1 text-[11px] font-semibold uppercase tracking-wide text-gray-900 md:text-[13px]">
                      {study.author}
                    </div>
                    <div className="text-[11px] text-gray-500 md:text-[13px]">{study.role}</div>
                  </motion.div>
                )}
              </AnimatePresence>
            </motion.div>
